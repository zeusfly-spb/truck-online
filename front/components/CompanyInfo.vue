<template>
  <v-col>
    <div v-if="companyInfo" class="d-flex justify-center">
      <strong>{{ companyInfo[0].value || "" }}</strong>
    </div>
    <template v-if="companyDetails">
      <div v-for="(item, index) in textDetails" :key="index">
        <DetailItem :content="item" :title="index" />
      </div>
      <div v-for="(item, index) in objectDetails" :key="index">
        <DetailItem :content="item" :title="index" />
      </div>
    </template>
  </v-col>
...
const companyInn = computed(
  () => authStore.user && authStore.user.company && authStore.user.company.inn,
);
...
const getInfo = async () => {
  ...
  const {
    data: { _rawValue },
  } = await useFetch(infoPath, {
    method: "POST",
    headers,
    body: { inn: companyInn.value },
  });
  companyInfo.value = _rawValue;
};
const companyDetails = computed(() => {
  const unchanged =
    companyInfo.value && companyInfo.value[0] && companyInfo.value[0].data;
  if (!unchanged) {
    return null;
  }
